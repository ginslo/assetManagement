...
@section('content')
  ...
  <div class="row">
    <div class="col-sm-8 col-md-offset-2">
			...
			<table>
				<tr>
					<th class="servtable">Domain Name: </th><td class="servtable">{{ $domain_name->name }} <a href="{{ route('domain_names.domain_name.edit', $domain_name->id) }}"><span title="Edit" style="float:right;"><i class="fa fa-pencil" aria-hidden="true"></i></span></a></td>
				</tr>
				<tr>
	        ...
	        <th class="servtable">Expiration Date: </th><td class="servtable">{{ $domain_name->expiration_date }}</td>
				...
				<tr>
	        <th class="servtable">User: </th><td class="servtable"><a href="{{ route('users.user.show', $domain_name->user->id) }}">{{ $domain_name->user->first_name }} {{ $domain_name->user->last_name }}</a></td>
				</tr>
				<tr>
	        <th class="servtable">Company: </th><td class="servtable"><a href="{{ route('companies.company.show', $domain_name->user->company->id) }}">{{ $domain_name->user->company->name }}</a></td>
				</tr>
				<tr>
					<th class="servtable">Registrar: </th><td class="servtable"><a href="{{ route('registrars.registrar.show', $domain_name->registrar->id) }}">{{ $domain_name->registrar->name }}</a></td>
				</tr>
				<tr>
