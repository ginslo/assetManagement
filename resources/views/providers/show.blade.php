...
<div class="row">
	<div class="col-sm-9 col-md-5 col-md-offset-3">
		...
		<h1><p style="float:right;"><a href="{{ route('providers.provider.edit', $provider->id) }}"><span title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i></span></a>
	...
	<div class="col-sm-9 col-md-6 col-md-offset-3">
		...
		<ul>
			@foreach($data_centers as $data_center)
				<li><a href="{{ route('data_centers.data_center.show', $data_center->id) }}">{{ $data_center->id }} {{ $data_center->name }}</a>
			@endforeach
		</ul>
		</div>
	</div>
