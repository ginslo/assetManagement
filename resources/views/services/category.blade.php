              ...
              <div class="ccm-block-page-list-wrapper">
                <div class="ccm-block-page-list-pages">
                  ...
                  @if(Auth::user()->is_admin == 1)
          					{{-- <a href="/products/product/{{ $service->id }}/edit"><span title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i></span></a> --}}
          				@endif
          			@endunless
          			 <p>{!! $category->description !!}</p>
                 <ul>
                 @foreach($products as $product)
                   <li><a href="/services/{{$product->category->slug}}/{{ $product->slug }}/">{{ $product->name }}</a></li>
                 @endforeach
                </ul>
              @endforeach
            </div>
