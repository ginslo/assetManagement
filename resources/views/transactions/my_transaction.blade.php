...
<div class="row">
	<div class="col-sm-9 col-sm-6 col-md-offset-4">
		<i class="fa fa-backward" aria-hidden="true"></i> <a href="{{ route('transactions.my_transactions.index') }}">All transactions</a>
    <h2>{{ $title }}</h2>
        ...
        <tr>
          ...
          <td class="servtable">{{ $transaction->transaction_no }}</td>
				...
				<tr>
					...
					<td class="servtable"><a href="{{ route('invoices.my_invoice.show', $transaction->invoice_id) }}">{{ $transaction->invoice_id }}</td>
					</tr>
					<tr>
        <tr>
