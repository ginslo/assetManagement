...
<div class="row">
	<div class="col-sm-10 col-md-8 col-md-offset-2">
		...
		<h1><p style="float:right;"><a href="{{ route('websites.website.edit', $website->id) }}"><span title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i></span></a></p>
		Website: {{ $website->name }}</h2>
		<table>
			<tr>
				<th class="servtable">Base Domain Name:</th>
				<td class="servtable"><a href="{{ route('domain_names.domain_name.show', $website->domain_name->id) }}">{{ $website->domain_name->name }}</a></td>
			</tr>
			<tr>
			...
			<tr>
		    <th class="servtable">Owned by:</th>
				<td class="servtable"><a href="{{ route('users.user.show', $users->id) }}">{{ $users->first_name }} {{ $users->last_name }}</a> (<a href="/companies/company/{{ $companies->id }}">{{ $companies->name }}</a>)</td>
			</tr>
			<tr>
